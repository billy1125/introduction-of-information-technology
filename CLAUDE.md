...
各章節另有對應的 Marp 投影片版本 `0X-XXX.slides.md`（目前涵蓋 00–04 章），撰寫規範見下方〈投影片（`.slides.md`）撰寫規範〉。
...
- **學習重點總結**：各章的最後一個小節固定為 `## {中文數字}、學習重點總結`，緊接在 `## 參考文獻` 之前，全專案格式一致（範本見 `04-Networks-and-Internet.md`〈二十五、學習重點總結〉）。寫法為：先一句固定引言「讀完本章後，你應該能夠理解以下核心概念，並將其應用於工業場域的思考與決策：」，接著以 5–6 個**粗體要點標題**（不使用 `###`，也不使用條列）分段，每個要點下方以一段散文說明該概念的內涵，並帶到它在工業場域的意義；要點是本章核心概念的收束與提煉，不是小節標題的複述。小節最後以 `> **銜接提示**` 收尾（01–06 章前指 07 章；07 章為全課程終點，不需銜接提示）。
...
- **避免跨章節重複**：各主題的權威章節如下，其他章節提到時應以一兩句話帶過並以連結指向對應章節，不要重新完整介紹一次。**新增趣味小知識或範例前，務必先搜尋該主題是否已在他章出現過**（例如 SQL／SEQUEL 的由來、大數據 3V 的來歷都已在 05 章）。
  ...
  - 工業4.0本身（定義、九大支柱、數位轉型、數位孿生）與智慧工廠整合案例 → `07-Industry-4.0-and-Smart-Manufacturing.md`
- **跨章連結格式**：連結文字用**完整檔名**（`[05-Information-Systems-and-Database.md](05-Information-Systems-and-Database.md)`）而非「05 章」；小節指涉用書名號〈〉寫在連結**外面**（如〈三、企業應用系統〉或〈六、2. 資料在流程中的角色〉），不使用 `.md#anchor`。四種既有句型：`> **延伸閱讀**`（本章點到為止，推給權威章）、`> **前情提要**`（回指先前章節）、`> **銜接提示**`（章末前指 07 章，01–06 章都應有）、章末斜體註記。一律使用 blockquote。範本見 `02-Computer-Structure.md`〈二十三、專業工業應用軟體〉。
