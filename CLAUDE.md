...
- **參考文獻**：各章 `## 參考文獻` 一律以 **APA 格式** 撰寫，全部為 **英文** 條目（無序清單，每筆一行），並依 **第一作者姓氏字母** 排序（同作者再依年份）。內容應兼顧 **經典教科書**（如 Tanenbaum、Silberschatz、Cormen 等公認課本）與 **原典或里程碑論文**（如 Turing 1936、Codd 1970、Cerf & Kahn 1974、Vaswani 2017 等），期刊、會議論文與技術標準若有 DOI 或穩定網址則附上。書名與期刊名用斜體，數量以各章約 20 筆以上為原則、與 `06-Data-Science-AI-and-Smart-Manufacturing.md` 的份量看齊。清單之後空一行、以 `---` 分隔，再接一段斜體結語（說明本章用途與延伸練習建議）。新增文獻時務必插入正確的字母位置，維持整份清單排序一致。
