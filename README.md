...
欄位空白者表示 **內容尚未定案、仍可能調整**，會隨著課程進行陸續標上 ✅；範圍相關的異動一律於課堂與 LINE 群組公告。
...
**這個儲存庫裡的文字，是作者與生成式 AI 協作的成果，全部經過作者逐份審閱之後才放上來。內容正確與否的責任在作者，不在工具。** AI 生成的內容仍可能夾帶錯誤或過時資訊，作者雖已盡力查核與編修，仍難保完全無誤。

| 項目 | 說明 |
|---|---|
| 使用工具 | 文字部分使用 OpenAI 的 ChatGPT 與 Anthropic 的 Claude、Claude Code；圖片部分一律以 ChatGPT 生成。模型版本隨產品更新而變動，故不逐一標註版本號 |
| 使用目的 | 章節草稿撰寫、既有文字的整理與潤稿、跨章節的名詞與格式一致性檢查、投影片與練習題初稿的產生，以及部分示意圖的生成 |
| 使用範圍 | AI 參與了各章正文、投影片（`0X-XXX.slides.md`）與練習筆記本（`notebooks/*.ipynb`）的草稿撰寫與潤稿，以及少數示意圖的繪製。**未參與** 的部分是：課程規劃與各章架構安排、評分與考試等行政規定、參考文獻的挑選、外部引用圖片的來源查證與授權判斷——這些一律由作者自行決定。教材中的數據、年代與史實由作者查證，AI 不作為事實來源 |
| 人工查核與修改 | 每一章正文、每一份投影片與筆記本都由作者逐段讀過才發布。具體到什麼程度：技術定義、年代與人物等事實逐項比對教科書或原始文獻；範例與情境全部改寫成製造業與工業工程的場景；章節取捨、敘述順序與用詞風格由作者決定；正文與練習筆記本中的程式碼一律由作者確認過才收錄。AI 生成的示意圖經作者檢核內容正確性後才收錄，並於圖片下方以圖說標註（例如「本圖由 ChatGPT 生成，已由作者審核內容正確性」）；引用自外部來源（如維基百科）的圖片則標註原始出處與連結，依其原始授權條款使用，不適用上方的整體授權聲明 |

**發現錯誤請告訴作者。** 任何人都可以在本儲存庫開 [Issue](https://github.com/billy1125/Introduction-of-Information-Technology/issues) 回報；修課同學也可以直接在課堂上提出，或寫信到 chohsunlu@saturn.yzu.edu.tw。
