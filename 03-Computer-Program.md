...
> 本章的目的不是把你訓練成程式設計師，而是讓你理解「程式是怎麼思考問題的」：掌握運算思維、讀懂基本程式邏輯、知道資料結構與演算法各自有什麼用。有了這些，你在數位化的工作環境裡才能跟 IT 與 AI 工程師溝通，也才有能力運用基本的自動化工具。
...
![1947 年哈佛 Mark II 電腦維修日誌，貼有造成故障的真實飛蛾標本](images/03-Computer-Program/first-computer-bug.jpg)
...
一條完整的生產線看似複雜，但透過問題分解，可以將其拆為：原料入庫 → 備料 → 加工站 A → 加工站 B → 品質檢測 → 包裝 → 出貨。每個步驟都有明確的輸入（上一站的半成品）與輸出（下一站的輸入），這種結構化的拆解方式，正是撰寫生產管理程式的基礎。
...
用工廠機台來比喻：「機台」可以設計成一個類別，定義它共同擁有的屬性（機台編號、目前狀態、良率）與方法（啟動、停機、記錄良率）。實際產線上的「A 線沖床」、「B 線車床」都是依照這個類別產生出來的物件，各自擁有獨立的屬性值，卻共用同一套方法。
...
> **趣味小知識**：堆疊的「push」與「pop」這兩個術語，據說正是來自這個「一疊盤子」的畫面——早期美式自助餐廳常見一種彈簧式疊盤機（spring-loaded plate dispenser）：你把盤子往下壓（push）放進去，彈簧會把它連同下面所有盤子一起頂住；抽走最上面一個（pop），彈簧又把剩下的盤子頂高一階。1950 年代的電腦科學家借用了這個生活中隨處可見的裝置，來命名堆疊「後進先出」的存取方式，比起憑空發明術語，直接借用日常經驗反而更容易讓人一聽就懂。
...
**映射（Map）** 又稱字典（Dictionary），是一種儲存 **鍵值對（Key-Value Pair）** 的資料結構：每個 **鍵（Key）** 對應一個 **值（Value）**，就像現實中的字典——查「apple」這個字（鍵），得到它的意思（值）。它內部通常以[雜湊表（Hash Table）](https://zh.wikipedia.org/zh-tw/雜湊表)實作。
...
> **銜接提示**：本篇（十四～十七）談的「資料驅動流程改善」「自動化思維」「工業4.0資料流」「ERP/MES 背後的程式邏輯」，會在全課程最後的 [06-Data-Science-AI-and-Smart-Manufacturing.md](06-Data-Science-AI-and-Smart-Manufacturing.md)〈五、工業4.0與智慧製造：把整條技術鏈放進工廠〉整合成一個完整的智慧工廠案例，和硬體、資訊系統、資料科學的內容放在一起。到時候可以回頭對照這裡學過的概念。
