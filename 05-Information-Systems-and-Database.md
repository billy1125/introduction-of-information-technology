...
**先建立一個心理準備**：把資訊系統切成作業、管理、策略三個層級，是為了「概念上好討論」而做的教科書式分類，方便你理解各種系統各自在忙什麼。但實務上你很難找到一套系統剛好只落在單一層級——現代資訊系統越做越龐大，往往同時橫跨好幾個層級，而且系統規模越大，越是包山包海。以本章後面〈三、企業應用系統〉會談到的 ERP 為例，它底層有處理進出貨、訂單、庫存異動的交易功能（作業層級），中間能產出各種產能與成本報表（管理層級），高層還能支援跨廠的產能配置決策分析（策略層級），一套系統就把三個層級全包了。因此，閱讀以下分類時請把它當成理解資訊系統的「地圖」，而不是現實中每套系統的硬性歸類。
