...
- 本章負責的是：從資料萃取知識——資料科學與 AI 找出規律，最後在工業4.0的智慧工廠落地
...
> **「資料很多」本身並不會自動變成價值——最後那一階「資訊→知識」還沒人接手**

---

## 卡住的最後一階：從資訊到知識

- **資訊** 回答「發生了什麼」：本班次不良率 3%、這家店上週來客數少兩成、這位病人血糖偏高
- **知識** 回答「為什麼、下次該怎麼辦」：轉速超過每分鐘 2000 轉不良率會上升、下雨加施工會掉客、某指標偏高要提前回診

> **前者是一筆筆的事實，後者是能重複套用的判斷原則**
...
| 侷限 | 具體情況 |
|------|---------|
| **看不完** | 一天讀不了幾百張報表，數萬個感測器每秒的數值更不可能用眼睛看 |
| **帶不走** | 判斷留在老師傅、資深店長、資深醫師腦袋裡，人一走就消失 |
| **不一定準** | 容易被少數印象深刻的案例牽著走，未必經得起統計檢驗 |

第二波 AI 的 **專家系統** 想把規則一條條寫死，但規則寫不完、還彼此衝突，這條路走不通。

---

## 換個方向：讓電腦自己找規律

**能不能不要由人寫規則，而是讓電腦直接從大量資料裡把規律找出來？**

這個想法很早就有人提出，但在沒有成熟的演算法、也沒有夠強的硬體之前，只能停留在紙上。資料早就在那裡；直到演算法成熟、運算能力跟上，這條路才走得通。

> **這正是資料科學登場的原因，也是 AI 沉寂數十年後再度崛起的起點——它們要解決的，就是階梯上最後、也最難的一階**
...
1. **採集**：把散在感測器、ERP／MES、電商平台的資料集中；先定義好要回答什麼問題，才知道該收什麼
2. **清洗**：補缺失、去重複、統一格式與單位、剔除異常值——最不起眼卻最花時間的一環
3. **分析**：用統計、分群、關聯分析或機器學習模型，從資料中找出規律
4. **視覺化**：畫成圖表或儀表板，讓沒有統計背景的人也看得懂
5. **決策**：把規律變成行動——調參數、排保養、改備料計畫

> **這五步很少一路順著走完，常常分析到一半才發現資料收得不夠，得回頭重來一輪**
...
> **最關鍵的同樣是前兩步——訓練資料不夠多、品質又不好，訓練出來的模型自然很兩光**
...
先給電腦一條公式的形狀（$x$ 切削速度、$y$ 刀具壽命，$a$、$b$ 是待定的 **參數**）：

$$
y = a \times x + b
$$

- 餵進實測紀錄：速度 80→176 分鐘、100→160 分鐘、120→144 分鐘……
- **每一筆資料，都等於對這條公式多加一個限制**
- 資料越多，能滿足所有限制的 $a$、$b$ 就被壓得越窄，最後收斂成 $y = -0.8x + 240$

> **公式的形狀由人決定，公式裡的數字由資料決定——這就是「訓練」**

---

## 為什麼 AI 有這麼多種演算法？

**因為「公式的形狀」是由人決定的**，而真實世界的關係很少是一條直線：

- 抓變數之間的線性關係 → **線性迴歸**
- 模仿人「一層層問是非題」→ **決策樹**
- 處理影像 → **卷積神經網路（CNN）**；處理語言 → **Transformer**

**選模型**（人挑形狀，靠領域理解）與 **訓練模型**（資料填數字，靠資料與算力）是兩件不同的事。

> **名字天差地遠，骨子裡都是同一件事：給出一種公式的形狀，再讓資料把參數定出來**

---

## 真實的模型只是放大同一件事

- 要調的參數不是 2 個，而是 **幾百萬到上兆個**
- 輸入不只一個變數，而是整張影像的 **上百萬個像素**
- 訓練完成後，這組數字就是電腦從資料中「學」到的規則
...
上一節說：公式的形狀由人給，裡面的數字由資料決定。但電腦不可能把所有可能的 $a$、$b$ 都試一遍。

它的做法是「**先猜、再改**」的循環：

- 隨便給一組數字當起點
- 拿去預測，量一下錯得多離譜
- 朝著讓錯誤變小的方向微調，重複成千上萬次
...
1. **準備訓練資料**：蒐集「輸入＋正確答案」成對的資料，例如標好良品／不良品的產品照片
2. **模型給出預測**：把輸入代進目前的參數；第一輪參數是隨機的，預測幾乎等於亂猜
3. **計算誤差**：用 **損失函數（Loss Function）** 把「錯得多離譜」量化成一個數字

---

## 訓練的六個步驟（下）

4. **調整參數**：往能讓誤差變小的方向挪一點；挪多少由 **學習率（Learning Rate）** 決定——太大會震盪，太小則訓練太慢
5. **重複循環**：再預測、再算誤差、再調整，跑上千到數百萬次，直到誤差幾乎不再下降
...
**第 5 步「誤差一路下降」藏著陷阱**：那個誤差是拿訓練資料算的，變小不代表真的學會——可能只是把細節硬背下來。
...
> **數位孿生的靈魂：先在分身上試，再對本尊動手——阿波羅 13 號當年用的是實體模擬艙，今天這個分身活在電腦裡，可以有一千個、隨時重來**
...
> **資訊科學的終極目標，不是讓電腦變得更聰明，而是讓人能夠用資料做出更好的決策——無論這個「人」是產線上的工程師、倉庫的物流主管，還是整個供應鏈的策略決策者**
