...
![h:600](images/03-Computer-Program/programmer-work.png)
...
- 2006 年在《ACM 通訊》發表短文，主張運算思維應像讀寫算一樣成為基本素養
...
**工業應用**：生產線可拆為 原料入庫 → 備料 → 加工 A → 加工 B → 檢測 → 包裝 → 出貨
...
- 廠房設備你不用知道機器怎麼運作的細節，只要知道「這台機器需要哪些原料？產出的零件是甚麼？」
- 「函數」正是抽象化的實踐：你不需要知道 `計算平均值()` 這個函數內部如何運作，只需要知道「給它一串數字，它會回傳平均值」
...
![h:600](images/03-Computer-Program/computational-thinking.png)
...
![h:600](images/03-Computer-Program/program-sequence.png)
...
![h:600](images/03-Computer-Program/datatype.png)
...
![h:600](images/03-Computer-Program/control-structure.png)
...
![h:600](images/03-Computer-Program/control-structure-mix.png)
...
- 函數把重複邏輯集中成一個可呼叫的單位，模組化把相關功能整理在一起

> **減少重複、提高可讀性，維護、測試與擴充都會變容易。**

---

<!-- ## 程式模組化概念 -->

![h:600](images/03-Computer-Program/program-marization.png)
...
**物件導向程式設計（Object-Oriented Programming, OOP）** 把資料與操作資料的函數包在一起管理。
...
![h:600](images/03-Computer-Program/why-data-structure.png)
...
![h:600](images/03-Computer-Program/array-list.png)
...
![h:600](images/03-Computer-Program/random-access-traversal.png)
...
![h:600](images/03-Computer-Program/stack.png)
...
![h:600](images/03-Computer-Program/queue.png)
...
![h:600](images/03-Computer-Program/set.png)
...
![h:600](images/03-Computer-Program/map.png)
...
![h:600](images/03-Computer-Program/tree.png)
...
![h:600](images/03-Computer-Program/line-search.png)
...
![h:600](images/03-Computer-Program/bubble-sort.png)
...
![h:600](images/03-Computer-Program/time-complexity.png)
...
| 資料結構 | 資料怎麼擺：擺對了後面每一步都省力 | 工單用佇列、料號查庫存用映射、BOM 用樹 |
| 演算法與效率 | 算不算得完：資料量放大十倍還撐不撐得住 | 工單排序、百萬筆料號查詢、排程的組合爆炸 |
...
![h:600](images/03-Computer-Program/industry-40.png)
...
本篇（十四～十七）談的資料驅動、自動化、工業4.0資料流、ERP/MES 程式邏輯，會在全課程最後的智慧工廠案例中，與硬體、資訊系統、資料科學內容整合在一起。
...
| 筆記本段落 | 對應本章 | 練習內容 |
|---|---|---|
| 第一篇：程式設計與運算思維 | 一～五節 | 良率分析四步驟、變數與型態、條件與迴圈、函數模組化、感測器資料處理 |
| 第二篇：資料結構基礎 | 七～九節 | 列表索引與走訪、堆疊與佇列的取出順序、集合去重與映射查詢 |
...
| 折線圖 | 隨時間變化的趨勢 | 每日良率趨勢 |
| 柱狀圖 | 不同類別的比較 | 各機台產量比較 |
| 柏拉圖 | 頻率排名＋累計比例 | 不良原因分析（80/20 法則） |
| 散布圖 | 兩變數的相關性 | 製程溫度與良率關係 |
| 管制圖 | 製程資料與管制界限 | SPC 品質管制 |
| 圓餅圖 | 各部分佔整體的比例 | 不良類型佔比（謹慎使用） |
...
- **異常標記**：明顯標示低於目標的時間點
- **趨勢線**：顯示整體走向
- 工具上 Excel 已足夠，要更靈活可用 Python 的 matplotlib，或 Power BI、Tableau
